# AGENTS.md — pi-openai-codex-usage

A [pi coding agent](https://github.com/earendil-works/pi-mono) extension that surfaces ChatGPT Codex subscription usage in the footer while an `openai-codex` provider model is active: multi-bucket limits (codex, spark, additional), earned reset credits (read + guarded consume), and a `/codex-usage` report with `--json`.
...
- `ui.select` resolves the chosen option **string** verbatim (no index, no truncation). Never recover choice identity by re-rendering label text — labels embedding live relative time (`formatReset`) drift under the user's dwell and the match silently fails. Build the labels array once (ordinal-prefix to keep duplicates distinct), map back with `indexOf`, and fail closed (not "cancelled") on a foreign string.
